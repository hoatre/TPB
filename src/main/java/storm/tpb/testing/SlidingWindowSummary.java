 ...
 */
public final class SlidingWindowSummary<T> implements Serializable {

    private static final long serialVersionUID = -2645063988768785810L;

    private SlotBasedSummary objCounter;
    private int headSlot;
    private int tailSlot;
    private int windowLengthInSlots;

    public SlidingWindowSummary(int windowLengthInSlots) {
//      if (windowLengthInSlots < 2) {
//      throw new IllegalArgumentException(
//          "Window length in slots must be at least two (you requested " + windowLengthInSlots + ")");
//    }
        this.windowLengthInSlots = windowLengthInSlots;
        this.objCounter = new SlotBasedSummary(this.windowLengthInSlots);

        this.headSlot = 0;
        this.tailSlot = slotAfter(headSlot);
    }

    public void incrementSummary(Transaction obj) {
        objCounter.incrementAmount(obj, headSlot);
    }
    ...
    public Map<Transaction, Long> getCountsThenAdvanceWindow() {
        Map<Transaction, Long> counts = objCounter.getTotal();
